...
export default function EditTaskForm() {
  ...
  if (users) {
    ...
    const options = useMemo(() => {
      return (
        users?.data?.map(
          (u: { full_name: string; email: string; id: string }) => ({
            value: u.id,
            label: u.full_name,
          })
        ) || []
  ...
  useEffect(() => {
    if (task) {
      ...
      reset({
        ...
        assignee: options.find((opt) => opt.value == task.assignee_id),
        dueDate: new Date(task.due_date),
        status: statusOptions.find((opt) => opt.value === task.status) || {},
        attachment: {},
      });
    }
  }, [task]);
